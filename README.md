...
![ログイン画面](docs/designs/auth_login.png)

### ホーム / すべての記事一覧

![ホーム / すべての記事一覧](docs/designs/desktop_article_list.png)

### 記事詳細ビュー

![記事詳細ビュー](docs/designs/desktop_article_detail_light.png)

### カレンダー

![カレンダー画面](docs/designs/calendar_view.png)

### タグ管理

![タグ管理画面](docs/designs/tag_management.png)

### 記事追加モーダル

![追加モーダル / 追加フォーム](docs/designs/add_article_modal.png)

### フィルタモーダル

![フィルタモーダル](docs/designs/filter_dialog.png)

### タグ追加モーダル

![タグ追加モーダル](docs/designs/tag_add_dialog.png)

### タグ統合モーダル

![タグ統合モーダル](docs/designs/tag_merge_dialog.png)

### タグ削除確認

![タグ削除確認ダイアログ](docs/designs/tag_delete_dialog.png)

### 記事削除確認

![記事削除確認ダイアログ](docs/designs/delete_article_dialog.png)

モバイル表示対応は未対応です。スマホ対応の検討内容は [docs/design/mobile-responsive.md](docs/design/mobile-responsive.md) に整理しています。
...
- フロントエンドは Node.js `22 LTS` で `npm ci` を実行し、`npm run typecheck`、`npm run test:unit`、`npm run build` で型チェック、domain / store の回帰、Vite ビルドを確認する
- バックエンドは `docker compose run --rm backend mvn clean compile spotbugs:check` と `docker compose run --rm backend mvn test` を実行し、静的解析、UT / IT、コンパイルエラー、設定崩れを検知する
- E2E は Playwright Chromium で P0 導線を確認する
...
- スマホ対応デザイン検討は [docs/design/mobile-responsive.md](docs/design/mobile-responsive.md) に整理しています
