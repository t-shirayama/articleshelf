...
ローカル開発用の初期ユーザーは `owner` / `password123` です。環境変数 `READSTACK_INITIAL_USERNAME` と `READSTACK_INITIAL_USER_PASSWORD` で変更できます。
...
- 認証関連の主な環境変数は `JWT_ACCESS_SECRET`, `AUTH_REFRESH_TOKEN_HASH_SECRET`, `AUTH_COOKIE_SAME_SITE`, `AUTH_COOKIE_SECURE`, `AUTH_CSRF_ENABLED`, `READSTACK_INITIAL_USERNAME`, `READSTACK_INITIAL_USER_PASSWORD`
