...
E2E は Playwright と Docker Compose を使い、登録、記事追加、ログアウト / ログイン、ユーザー分離の主要導線に加えて、他ユーザーによる更新 / 削除拒否も確認します。Playwright は必要に応じて `docker compose up --build` を起動し、Compose は `/actuator/health` でバックエンドの起動完了を待ってからフロントエンドを起動します。ローカルで既に起動中のサーバーがある場合だけ再利用します。
