...
- リポジトリ: https://github.com/t-shirayama/articleshelf
- 公開版は画面の「登録」からユーザーを作成すると試せます。無料枠構成のため、初回アクセス時にバックエンドの起動待ちが発生することがあります。
- ローカルで試す場合は `docker compose up --build` で起動し、`http://localhost:5173` を開きます。詳しい手順は [使い方](#使い方) を参照してください。
...
- `Keep Render Awake` workflow は 10 分ごとに Render backend の `/actuator/health` を ping する
