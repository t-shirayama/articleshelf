...
サンプル投入コマンドは `sample` / `password123` のユーザーを作成または再利用し、サンプル記事と未使用タグを追加します。ユーザー名や API URL は `READSTACK_SAMPLE_USERNAME`, `READSTACK_SAMPLE_PASSWORD`, `READSTACK_SAMPLE_API_BASE_URL` で変更できます。
...
- 認証関連の主な環境変数は `JWT_ACCESS_SECRET`, `AUTH_REFRESH_TOKEN_HASH_SECRET`, `AUTH_COOKIE_SAME_SITE`, `AUTH_COOKIE_SECURE`, `AUTH_CSRF_ENABLED`, `READSTACK_INITIAL_USER_ENABLED`, `READSTACK_INITIAL_USERNAME`, `READSTACK_INITIAL_USER_PASSWORD`
...
coverage report はフロントエンドが `frontend/coverage/`、バックエンドが `backend/target/site/jacoco/` に出力されます。
