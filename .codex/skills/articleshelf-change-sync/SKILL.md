...
5. Choose verification based on risk:
   ...
   - Before and after running pre-push E2E smoke or Playwright checks that start the dedicated stack, stop stale containers with `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans`, then ensure TCP ports `localhost:4173` and `localhost:18080` are not occupied by another dev server or manually-started process. Do not auto-stop non-E2E processes. Use `PLAYWRIGHT_USE_EXISTING_SERVER=1` only for an intentionally prepared E2E server.
