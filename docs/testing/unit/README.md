...
重要ロジックだけを切り出した focused coverage は `npm run test:unit:coverage:focused` で実行し、article domain helper、主要 composable、Markdown / API / JWT utility の coverage を個別に確認する。
...
- UT は既存データや外部状態に依存させず、ケースごとに入力値を明示する
- 外部 I/O を伴う入力は test double や固定値へ置き換える
- 時刻、認証状態、API client、OGP 取得結果など、結果に影響する値はテスト内で固定する
- 外部サイトの OGP 取得安定性に UT を依存させない

## 7. CI での扱い

- `backend-unit` / `frontend-unit` で backend domain / application UT と frontend UT を coverage 付きで実行する
- frontend は Vitest coverage-v8 の text summary を出力し、lines 43%、statements 42%、functions 35%、branches 29% 未満を失敗にする
- frontend focused coverage は `test:unit:coverage:focused` で対象ファイルだけを評価し、lines / statements 85%、functions 90%、branches 70% 未満を失敗にする
