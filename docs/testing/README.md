...
Chrome 拡張機能の手動確認では、repository root から `docker run --rm -v ${PWD}:/workspace -w /workspace/chrome-extension node:24-alpine sh -c "npm ci && npm run build"` を実行し、ローカル確認用 zip を `frontend/public/downloads/articleshelf-chrome-extension-local.zip` に生成する。`docker compose up` などで frontend を `http://localhost:5173` に起動し、Help ダイアログから local zip をダウンロードして展開したうえで Chrome の `Load unpacked` から `articleshelf-chrome-extension-local/` を読み込む。通常の `https` ページを開いた状態で `Open draft in ArticleShelf` が `http://localhost:5173/articles?source=extension...` を開き、追加モーダルへ値を流せることを見る。
