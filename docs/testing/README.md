...
- pre-push や E2E smoke の前後で、hook は専用 E2E stack を `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` で止める。その後 `localhost:4173` / `localhost:18080` の TCP port 使用状況を確認し、HTTP 応答がない待受も検出する。Docker 停止直後の port 解放遅延に備えて短時間待機し、それでも port が埋まる場合は、別の dev server や手動起動 process を止める。hook は ArticleShelf 専用 stack 以外の process を自動停止しない。port が埋まったままだと Playwright webServer が起動できず、push は hook 失敗として止まる
