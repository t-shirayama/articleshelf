...
Chrome 拡張機能の手動確認では、repository root から `docker run --rm -v ${PWD}:/workspace -w /workspace/chrome-extension node:24-alpine sh -c "npm ci && npm run build"` を実行し、ローカル確認用 zip を `frontend/public/downloads/articleshelf-chrome-extension-local.zip` に生成する。`docker compose up` などで frontend を `http://localhost:5173`、backend を `http://localhost:8080` に起動し、Help ダイアログから local zip をダウンロードして展開したうえで Chrome の `Load unpacked` から `articleshelf-chrome-extension-local/` を読み込む。通常の `http` / `https` ページを開き、初回ログイン後に popup だけで未読保存、既読保存、登録済み記事の既読 / 未読更新ができることを見る。
