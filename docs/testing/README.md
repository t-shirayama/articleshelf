...
| E2E | ブラウザ上の主要ユーザーシナリオを検証 | 少数精鋭 | PR、main push、明示 opt-in、リリース前 |
...
- pre-push は develop などの通常 push では E2E smoke を既定で走らせず、targeted unit / backend check を優先する。E2E smoke は `main` push、または `ARTICLESHELF_PRE_PUSH_E2E=1` を指定した明示 opt-in のときだけ実行する
- pre-push や E2E smoke の前後で、hook は専用 E2E stack を `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` で止める。その後 `localhost:4173` / `localhost:18080` の TCP port 使用状況を確認し、HTTP 応答がない待受も検出する。Docker 停止直後の port 解放遅延に備えて短時間待機し、それでも port が埋まる場合は、別の dev server や手動起動 process を止める。hook は ArticleShelf 専用 stack 以外の process を自動停止しない。port が埋まったままだと Playwright webServer が起動できず、E2E smoke は hook 失敗として止まる
...
ローカル hook は CI の代替ではなく早期検知のために使う。`.githooks/pre-commit` は型チェックと静的な運用ガードに限定し、`.githooks/pre-push` は変更パスに応じて frontend targeted unit や backend `docker compose run --rm backend mvn test -Dtest=CleanArchitectureDependencyTest`、`docker compose run --rm backend mvn test spotbugs:check` を実行する。E2E smoke は通常の develop push では実行せず、`main` push または `ARTICLESHELF_PRE_PUSH_E2E=1` の明示 opt-in に限定する。
...
pre-push の E2E smoke を実行する場合は、Playwright 起動前と終了後に `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` を実行して専用 stack を掃除する。掃除後も `localhost:4173` または `localhost:18080` の TCP port 使用を検出した場合は、Docker 停止直後の port 解放遅延に備えて短時間待機する。それでも解放されない場合は、別の dev server や手動起動 process を止めてから再実行する。Windows では `netstat -ano | findstr ":4173"` / `netstat -ano | findstr ":18080"` で使用 process を特定する。hook は ArticleShelf 専用 stack 以外の process を自動停止しない。`PLAYWRIGHT_USE_EXISTING_SERVER=1` は専用の準備済み E2E server を使う明示的な手動確認に限る。
...
Chrome 拡張機能導線では、PKCE 認可、token exchange、extension token の scope enforcement、未読登録、既読登録、登録済み記事の status update、期限切れ / revoke 済み token の拒否を確認する。互換 route として `AuthScreen.test.ts` で `returnTo` 付きログイン後の復帰を確認し、`useArticleCreateForm.test.ts` で拡張機能から渡した URL / title seed が追加モーダルへ反映されることも維持する。
Chrome 拡張機能の手動確認では、repository root から `docker run --rm -v ${PWD}:/workspace -w /workspace/chrome-extension node:24-alpine sh -c "npm ci && npm run build"` を実行し、ローカル確認用 zip を `frontend/public/downloads/articleshelf-chrome-extension-local.zip` に生成する。`docker compose up` などで frontend を `http://localhost:5173`、backend を `http://localhost:8080` に起動し、Chrome 拡張機能ダイアログから local zip をダウンロードして展開したうえで Chrome の `Load unpacked` から `articleshelf-chrome-extension-local/` を読み込む。通常の `http` / `https` ページを開き、初回ログイン後に popup だけで未読保存、既読保存、登録済み記事の既読 / 未読更新ができることを見る。
