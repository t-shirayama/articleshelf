...
pre-push の E2E smoke は、Playwright 起動前と終了後に `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` を実行して専用 stack を掃除する。掃除後も `localhost:4173` または `localhost:18080` の TCP port 使用を検出した場合は、別の dev server や手動起動 process を止めてから再実行する。Windows では `netstat -ano | findstr ":4173"` / `netstat -ano | findstr ":18080"` で使用 process を特定する。hook は ArticleShelf 専用 stack 以外の process を自動停止しない。`PLAYWRIGHT_USE_EXISTING_SERVER=1` は専用の準備済み E2E server を使う明示的な手動確認に限る。
