...
- pre-push や E2E smoke の前後で、hook は専用 E2E stack を `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` で止める。それでも `localhost:4173` / `localhost:18080` が埋まる場合は、別の dev server や手動起動 process を止める。port が埋まったままだと Playwright webServer が起動できず、push は hook 失敗として止まる
...
pre-push の E2E smoke は、Playwright 起動前と終了後に `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` を実行して専用 stack を掃除する。掃除後も `localhost:4173` または `localhost:18080` の既存応答を検出した場合は、別の dev server や手動起動 process を止めてから再実行する。`PLAYWRIGHT_USE_EXISTING_SERVER=1` は専用の準備済み E2E server を使う明示的な手動確認に限る。
