...
更新時は `docker compose -f docker-compose.e2e.yml up --build -d` でアプリを起動し、`http://localhost:4173` / `http://localhost:18080` を使って `frontend` で `npm run capture:designs` を実行します。
...
- `screenshots/capture-designs/1920x1080/auth_login.png`: ログイン画面
  ...
  - フォームカードは背景より少し強いコントラストで自然に分離し、操作開始位置を見つけやすくする
  - 切り替え領域は補助説明文を置かず、ラベルとボタンだけで簡潔に見せる
  - 左側モックアップ UI は最近保存カードを中心に、進捗と人気タグの重なりを弱めて読みやすく配置する
...
- `screenshots/capture-designs/1920x1080/auth_register.png`: ユーザー登録画面
  ...
  - ログイン画面と同じカードコントラストを保ち、長い補助文やエラー表示が出ても余白が崩れない
  - ログイン / 登録切り替えに冗長な説明を置かず、選択状態をボタン側で伝える
  - 左側モックアップ UI の重なりを整理し、フォーム側への視線を邪魔しない
...
- `screenshots/capture-designs/1920x1080/help_dialog.png`: Chrome 拡張機能ダイアログ
  - 左サイドバー下部の Chrome 拡張機能ボタンから開く
  - Chrome 拡張機能の役割、ダウンロード導線、読み込み手順を表示
  - Chrome Developer mode での読み込み手順と更新方法を表示し、ダウンロードボタンは濃い背景と白文字で表示
...
- `screenshots/capture-designs/1920x1080/add_article_modal.png`: 記事追加モーダル
  - 初期表示では URL 入力と保存を主役にし、プレビュー領域を表示
  - タイトル、タグ、あとで読む、既読日、メモは折りたたみ詳細から必要に応じて編集
  - 閉じる・保存ボタン
...
- `screenshots/capture-designs/430x932/mobile_add_article_modal.png`: モバイル版 記事追加モーダル
  ...
  - URL 入力とプレビューを先に見せ、詳細項目は折りたたみで表示
