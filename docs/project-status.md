...
- Playwright による登録、重複 URL、詳細編集、削除、既読 / 未読切り替え、複合フィルタ、ログアウト / ログイン、ユーザー分離の E2E
...
- Playwright E2E script、Vitest UT script、バックエンド UT / IT を追加済みです。CI は `npm run typecheck`、`npm run test:unit`、`npm run build`、`docker compose run --rm backend mvn clean compile spotbugs:check`、`docker compose run --rm backend mvn test`、`npm run test:e2e` を実行します
...
- PostgreSQL 18 公式イメージはデータ永続化パスの標準が `/var/lib/postgresql/data` から `/var/lib/postgresql` へ変わっているため、Compose の volume mount も新仕様に合わせています
