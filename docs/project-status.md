...
- GitHub Actions でフロントエンドの `typecheck` を独立 step として実行
...
- Playwright E2E script、Vitest UT script、バックエンド UT / IT を追加済みです。CI は `npm run typecheck`、`npm run test:unit`、`npm run build`、`docker compose run --rm backend mvn test`、`npm run test:e2e` を実行します
