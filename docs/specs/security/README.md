...
- refresh token rotation は pessimistic lock と条件付き update で atomic に行い、並行 refresh で複数 replacement token が有効化されないようにする
...
- Supply chain security は Dependabot、Dependency Review、CodeQL、Trivy で分担する。Dependency Review は PR の依存差分、CodeQL は Java / JavaScript / TypeScript の静的解析、Trivy は filesystem と backend Docker image の high / critical vulnerability、secret、misconfiguration を確認する
