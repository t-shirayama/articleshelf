...
- 現行の Render Free 運用では `ARTICLESHELF_OGP_REQUIRE_PROXY_IN_PROD=false` とし、production profile でも app-level guard で OGP outbound access を防御する
- dedicated outbound proxy を導入した production profile では `ARTICLESHELF_OGP_REQUIRE_PROXY_IN_PROD=true` と `ARTICLESHELF_OGP_PROXY_URL` を設定し、proxy / firewall 側で `169.254.169.254`、`100.100.100.200`、private network 宛て egress を遮断する
- Java HttpClient は検証後の接続で再解決される余地が残るため、app-level guard だけでは TOCTOU を完全には防げない。現行 Render Free 運用ではこの残余リスクを受け入れる
...
- production では `ProductionEnvironmentValidatorTest` で `ARTICLESHELF_OGP_REQUIRE_PROXY_IN_PROD=true` のときだけ `ARTICLESHELF_OGP_PROXY_URL` を必須にすることを確認する。現行 Render Free 運用では app-level SSRF guard の test を維持し、dedicated outbound proxy 導入後の運用 smoke check では proxy / firewall の deny rule で metadata endpoint と private network 宛て egress が遮断されることを確認する
