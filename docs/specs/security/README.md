...
- Chrome 拡張機能は token、cookie、CSRF token を保存せず、`chrome.storage` も使わない。拡張機能から渡すのは `articleUrl` と `articleTitle` だけに絞り、Web アプリ側で `http` / `https` URL を再検証したうえで追加モーダルへ流す。ArticleShelf 接続先は本番用とローカル確認用の配布物ごとに固定する
