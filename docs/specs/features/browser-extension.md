...
- manifest の権限は `activeTab` と `storage` のみにする
- popup は現在アクティブなタブの `url` と `title` を取得する
- ArticleShelf 接続先 URL は popup で変更でき、`chrome.storage.sync` に保存する
- 接続先 URL の既定値は `https://articleshelf.pages.dev`
- 接続先 URL は `http` / `https` のみ許可し、path、query、hash は保存時に取り除く
...
- build script は `dist/articleshelf-chrome-extension/` と `dist/articleshelf-chrome-extension.zip` を生成する
- 開発便宜のため、生成した zip は `frontend/public/downloads/articleshelf-chrome-extension.zip` にも同期する
