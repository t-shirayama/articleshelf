...
- Chrome 拡張機能は通常 Web JWT を使わず、Authorization Code + PKCE で取得した拡張機能専用 opaque token を `chrome.storage.local` に保存する。scope は記事保存操作だけに限定し、refresh token は発行しない
- cookie 認証を使う state-changing endpoint は `@CookieCsrfProtected` を付け、controller ごとの手動 CSRF validator 呼び出しに依存しない
...
- 登録 / ログインの公開 API は backend DB の `auth_rate_limit_buckets` を共有ストアにしたレート制限で保護する。client IP は Spring が確定した remote address を使い、複数 backend instance でも同じ key の試行回数を共有する
- `AuthService` は orchestration に集中し、refresh token rotation は `RefreshTokenRotationService`、初期管理ユーザー作成は `InitialUserProvisioner` を Spring bean injection で受け取る
...
- [Chrome 拡張機能仕様](../features/browser-extension.md)
