...
- `article_tags` は `user_id` を持つ join table とし、application / repository 層と DB 複合 FK の両方で article / tag の user mismatch を防ぐ
- API の検索・詳細・更新・削除はすべて認証ユーザーの `user_id` でスコープする
- ユーザー間で同じ URL や同じタグ名を登録できる
- local dev では `SameSite=Lax`, `Secure=false`, CSRF 無効を既定とする
- 公開環境では refresh / logout が cookie 認証ベースになるため CSRF token を必須にする
- 公開環境で cross-site 構成にする場合は `SameSite=None; Secure` と CSRF token を必須にする
- 現行実装は Flyway migration を適用したうえで JPA `validate` を使い、既存開発 DB の補助として起動時に初期 owner ユーザーへの owner 付与も維持する
...
- HS256 の発行 / 検証、`alg` header の扱い、`exp` 検証は `spring-security-oauth2-jose` に委譲し、独自の JWT 署名・payload parse 実装は持たない
- 有効期限: 15 分
- 保持場所: フロントエンドのメモリ上
...
現行実装では `article_tags` に `user_id` を持たせ、`PRIMARY KEY (user_id, article_id, tag_id)` と複合 FK で DB 層でも user mismatch を防ぐ。
...
現行実装では `JpaArticleRepository.resolveArticleTagEntities` で `article.userId` と `tag.userId` の一致を検証し、さらに `article_tags` の複合 FK で DB でも不正な組み合わせを拒否する。
...
- JWT は `JwtTokenService` から Spring Security JOSE の `NimbusJwtEncoder` / `NimbusJwtDecoder` を使って HS256 の発行 / 検証を行う
...
- CSRF:
  ...
  - production profile では `AUTH_CSRF_ENABLED=true` を必須にし、`false` の場合は起動エラーにする
...
| 設定 | デフォルト | 説明 |
| --- | --- | --- |
| `AUTH_COOKIE_SAME_SITE` | `Lax` | local dev 既定。無料枠 cross-site 公開では `None` に変更する |
| `AUTH_COOKIE_SECURE` | `false` | local dev 既定。本番は `true`。`SameSite=None` の場合は必ず `true` |
| `AUTH_CSRF_ENABLED` | `false` | local dev 既定。本番は `true` 必須 |
...
`AUTH_COOKIE_SAME_SITE=None` は frontend と API が別 site になる場合の推奨値とする。
同一 site 配信では `Lax` も選択可能だが、production profile では CSRF は常に有効にする。
...
- production profile では `AUTH_CSRF_ENABLED=false` の場合は起動エラーにする
- production profile では `FRONTEND_ORIGIN` 未設定を起動エラーにする

## 10. Migration 方針

現行実装は Flyway を導入し、`V1__baseline_schema.sql` で `users` / `refresh_tokens` / user scoped unique 制約 / `article_tags.user_id` / 複合 FK / 主要 index を管理する。
JPA は `spring.jpa.hibernate.ddl-auto=validate` に切り替え、起動時の `ReadStackDataInitializer` は既存開発 DB で `user_id` が未設定の legacy 記事・タグを owner へ寄せる補助に限定する。

移行上の注意:

1. Flyway は `baseline-on-migrate=true` と `baseline-version=0` で既存開発 DB にも適用する
2. migration 内で `articles.url` / `tags.name` の旧 unique 制約を整理し、user scoped unique へ寄せる
3. migration 内で `article_tags.user_id` を backfill し、複合 FK と `ON DELETE CASCADE` を追加する
4. auth 導入前の極端に古い DB で `users` が存在しないまま legacy データだけある場合は、自動補完だけでは足りないため DB 再作成または手動移行を前提とする

## 11. テスト観点

- JWT access token は HS256 で発行され、`sub`, `email`, `roles`, `iat`, `exp`, `jti` を保持する
- 改ざん token、期限切れ token、想定外 `alg` header の token は認証されない
- 未ログインでは記事 API にアクセスできない
- 登録後に access token と refresh cookie が発行される
- login 後に自分の記事だけ取得できる
- ユーザー A の記事 ID をユーザー B が詳細取得しても見えない
- ユーザー A の記事 ID をユーザー B が更新・削除しても `404` になる
