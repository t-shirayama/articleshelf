...
- refresh 処理の手順:
  1. cookie の refresh token を hash 化し、`refresh_tokens.token_hash` で検索する
...
実装上は application 層の検証を必須とし、DB 制約は防御的な二重化として扱う。
...
| 設定 | デフォルト | 説明 |
| --- | --- | --- |
| `AUTH_COOKIE_SAME_SITE` | `Lax` | 同一 site / custom domain 運用の既定値 |
