...
- `article_tags` も `user_id` を持ち、DB の複合 FK で article / tag の user mismatch を防ぐ
- API の検索・詳細・更新・削除はすべて認証ユーザーの `user_id` でスコープする
- ユーザー間で同じ URL や同じタグ名を登録できる
- MVP 公開時は無料枠の cross-site 構成を許容し、refresh cookie は `SameSite=None; Secure` と CSRF token を必須にする
...
- 形式: 256 bit 以上の CSPRNG で生成したランダムな不透明 token
...
- Cookie 属性:
  - `HttpOnly`
  - `Secure`
  - MVP 公開環境では cross-site 構成を許容するため `SameSite=None; Secure` を既定とする
  - custom domain / reverse proxy で frontend と API を同一 site に寄せられる場合のみ `SameSite=Lax` へ切り替え可能とする
...
DB 層でも user mismatch を防ぐため、`article_tags` に `user_id` を持たせる。
...
実装上は application 層の検証を必須とし、DB 制約は最後の防御線として扱う。
...
- CSRF:
  ...
  - MVP 公開環境では cross-site cookie を許容するため、cookie を使う refresh / logout は double submit cookie 方式の CSRF token を必須にする
...
MVP 公開環境では cross-site 構成を許容する案 B を採用し、次を必須とする。
...
| 設定 | デフォルト | 説明 |
| --- | --- | --- |
| `AUTH_COOKIE_SAME_SITE` | production: `None`, local dev: `Lax` | 無料枠 cross-site 公開では `None` を既定にする |
| `AUTH_COOKIE_SECURE` | 本番は `true`、local dev は `false` | `SameSite=None` の場合は必ず `true` |
| `AUTH_CSRF_ENABLED` | production: `true`, local dev: `false` | `SameSite=None` または cross-site cookie 運用では `true` 必須 |
