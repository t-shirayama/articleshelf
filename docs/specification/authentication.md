...
ReadStack はユーザーごとに記事、タグ、メモ、既読履歴を分離する。
...
- 通常起動では初期ユーザーを自動作成しない。初期 ADMIN が必要な検証環境だけ `READSTACK_INITIAL_USER_ENABLED=true` を明示する
- 登録 / ログインの公開 API は backend の in-memory レート制限で保護する。単一インスタンス向けの簡易制限とし、複数インスタンスでは共有ストアまたは proxy 側制限を別途使う
...
- `READSTACK_INITIAL_USER_ENABLED=true` の場合だけ、起動時に `READSTACK_INITIAL_USERNAME` / `READSTACK_INITIAL_USER_PASSWORD` の ADMIN ユーザーを作成または再利用する
