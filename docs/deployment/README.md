...
![ReadStack システム構成図](images/readstack-deployment-overview.svg)

上の図は、公開構成の全体像と、`main` 反映後に GitHub Actions から各公開先へ流す最終形を示す。
現時点の初期運用は `7.1` の通り Cloudflare Pages / Render の Git 連携でも始められ、CI / CD を分離した段階導入も許容する。
...
`2. 最終構成` の図では GitHub Actions から frontend / backend へ流す完成形を示しているが、初期公開ではこの Git 連携ベース運用から入ってよい。
