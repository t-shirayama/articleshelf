...
```env
ARTICLESHELF_INITIAL_USER_ENABLED=false
```
...
```env
ARTICLESHELF_AUTH_RATE_LIMIT_ENABLED=true
ARTICLESHELF_LOGIN_RATE_LIMIT_CAPACITY=5
ARTICLESHELF_LOGIN_RATE_LIMIT_WINDOW_SECONDS=60
ARTICLESHELF_REGISTER_RATE_LIMIT_CAPACITY=3
ARTICLESHELF_REGISTER_RATE_LIMIT_WINDOW_SECONDS=600
```
...
articleshelf-backend-0.1.0.jar created
Docker image built
Service starting
```
...
[ ] ARTICLESHELF_INITIAL_USER_ENABLED=false を設定した
