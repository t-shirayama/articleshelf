...
- 認証追加後、ユーザー A がユーザー B の記事を参照・更新・削除できないことを IT / E2E で確認する: 参照分離は達成、更新・削除の追加ケースは拡張候補
- DB 初期化、テストデータ、stub の運用が文書化されている: 一部達成。H2 / MockMvc / 一意メール / 一意 URL の方針を記載済み
