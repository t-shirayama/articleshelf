...
- GitHub Releases から配布 zip を取得し、Chrome の Developer mode で拡張機能を読み込める利用者

## 利用者導線

- 利用者は記事ページを開いた状態で拡張機能 popup を開く
- popup で ArticleShelf の接続先 URL を確認または変更できる
...
- ArticleShelf の接続先 URL を popup 内で設定できる
...
- 拡張機能 storage に保存する情報は ArticleShelf 接続先 URL に限定される
