...
- production profile では OGP fetch を `ARTICLESHELF_OGP_PROXY_URL` で指定した dedicated outbound proxy 経由にし、proxy 側で metadata endpoint と private network 宛て egress を遮断する
...
- Java HttpClient が接続時に再解決する余地は app だけで完全には消せないため、network egress control を proxy / firewall で補完する必要がある
