...
- `chrome-extension/`: 現在タブの `url` / `title` を使い、ArticleShelf へ直接保存する Chrome 拡張機能。仕様は [Chrome 拡張機能仕様](../../specs/features/browser-extension.md) を正本とする
...
- extension direct save: 拡張機能の PKCE 認可画面、未認証時の `returnTo` 復帰、互換 route の query consume は [Chrome 拡張機能仕様](../../specs/features/browser-extension.md) に従う
