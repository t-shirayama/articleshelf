...
- domain port のメソッド引数や戻り値に application DTO / command / query model を混ぜない。repository や domain service が共有する検索条件、ページング、sort key は domain 側の型として定義し、application はそれを組み立てて渡す
...
- `com.articleshelf.domain.article`
  ...
  - `ArticleListQuery` (repository port で共有するページング / sort 条件)
...
- `Article` は optimistic locking 用の `version` を持ち、`updateContent`、`changeStatus`、`changeFavorite`、`changeRating`、`replaceTags` で更新時の状態変更を表現し、アプリケーション層が更新用に集約を丸ごと作り直さない
...
- `UpdateArticleUseCase` は保存前に client version と current article version を照合し、JPA `@Version` と `saveAndFlush` を併用して lost update を `ARTICLE_VERSION_CONFLICT` へ変換する。URL 重複確認は transaction 内、OGP fetch は transaction 外で行い、stale update では外部 fetch を走らせない
- タグ管理ユースケースは `TagService` に分け、コントローラーも各サービスへ直接委譲する
- 永続化と記事一覧の基本検索条件（ステータス、単一タグ、検索語、お気に入り）は `ArticleRepository` / `JpaArticleRepository` 経由で扱い、domain 側の `ArticleListQuery` が表す `page` / `size` / `sort` は infrastructure 側で PostgreSQL の `LIMIT` / `OFFSET` / `ORDER BY` に変換する。application 独自の request DTO や query object をそのまま domain port へ流し込まない。タグ一覧・名称変更・マージ・未使用削除は `TagRepository` / `JpaTagRepository` 経由で扱う
- 認証ユースケースは `AuthService` を入口にしつつ、refresh token rotation は `RefreshTokenRotationService`、初期管理ユーザー作成は `InitialUserProvisioner` に分ける。`AuthService` はこれらを constructor injection で受け取り、自身は register / login / logout 系 orchestration に集中する。JPA Entity、Spring Data Repository、JWT発行、refresh token hash、password encoder は application 層のポート越しに扱う。JWT の署名・検証・`exp` 検証・`alg` 扱いは自前実装せず、`spring-security-oauth2-jose` の encoder / decoder に委譲する
- 認証ユースケースと `JwtTokenService` の時刻、UUID、CSRF token 用 random source は `Clock`、`IdGenerator`、`SecureRandom` Bean として注入し、access token の `iat` / `exp` / `jti`、token rotation、invalidation のテストで固定値を使えるようにする
