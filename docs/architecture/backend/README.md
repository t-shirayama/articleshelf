...
- `com.articleshelf.application.article`
  ...
  - `AddArticleUseCase` / `UpdateArticleUseCase` / `DeleteArticleUseCase`
...
- 記事ユースケースは `AddArticleUseCase`、`UpdateArticleUseCase`、`DeleteArticleUseCase`、`SearchArticlesQuery`、`FindArticleQuery` に分け、`ArticleService` は既存 Controller 契約を保つ薄い facade として各ユースケースへ委譲する
...
- 永続化と記事一覧の基本検索条件（ステータス、単一タグ、検索語、お気に入り）は `ArticleRepository` / `JpaArticleRepository` 経由で扱い、タグ一覧・名称変更・マージ・未使用削除は `TagRepository` / `JpaTagRepository` 経由で扱う
