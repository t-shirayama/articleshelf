...
- バックエンドは OGP 取得の port / adapter 分離を進めました。次はユースケース分割、`TagRepository` 分離、Web DTO と application DTO の境界整理が候補です
- フロントエンドは `App.vue` の薄い shell 化、feature-oriented な記事管理構成、記事操作 / タグ操作 / 詳細フォームの feature composable 分離、pure function 分離、feature 固有 data 分離、API client / adapter 分離、i18n locale 分割、CSS ファイル分割を進めました
