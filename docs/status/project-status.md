...
- 登録 / ログイン公開 API に Render 無料枠の単一インスタンス運用を前提にした in-memory レート制限を追加
...
- GitHub Actions でフロントエンドの `typecheck` とバックエンドの `spotbugs:check` を独立 step として実行し、backend unit では domain / application line coverage 55% の閾値を確認
