...
- 通常起動では初期データを投入せず、`READSTACK_INITIAL_USER_ENABLED=true` の検証環境だけ初期 ADMIN を作成し、既存記事 / タグの `user_id` 未設定データへ owner を付与
...
- 無料枠公開・CI/CD構成は `docs/deployment/free-deployment.md` に整理済み。初期公開は Cloudflare Pages + Render Free Web Service + Supabase Free PostgreSQL を前提にし、`PORT` 対応、Actuator health の公開設定、managed DB 向け JDBC URL 前提、production profile の起動検証は実装済みで、deploy workflow だけ未着手
