...
- 無料枠公開構成は `docs/deployment/README.md` に整理済み。Cloudflare Pages + Render Free Web Service + Supabase Free PostgreSQL の公開構成でデプロイ完了済みで、`PORT` 対応、Actuator health の公開設定、managed DB 向け JDBC URL 前提、production profile の起動検証は反映済み。公開反映は Cloudflare Pages / Render の自動デプロイで行う
...
- 退会後データの物理削除ポリシー
- データエクスポート
- 公開後の health check / smoke test 運用整理
...
2. 公開後の health check / smoke test の運用方針を固める
