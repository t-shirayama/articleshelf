...
- GitHub Actions の `keep-render-awake.yml` による 10 分間隔の Render `/actuator/health` ping
...
- 無料枠公開構成は `docs/deployment/README.md` に整理済み。Cloudflare Pages + Render Free Web Service + Supabase Free PostgreSQL の公開構成でデプロイ完了済みで、`PORT` 対応、Actuator health の公開設定、managed DB 向け JDBC URL 前提、production profile の起動検証、GitHub Actions による Render keep-awake は反映済み。公開反映は Cloudflare Pages / Render の自動デプロイで行う
...
- `docs/designs/screenshots/` は静的モック固定ではなく、現行実装を撮り直して同期する運用へ移行したため、UI 変更時は project skill `articleshelf-design-capture` の利用を前提とします
