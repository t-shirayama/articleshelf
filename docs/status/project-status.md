...
- フロントエンドは会社プロジェクト向けサンプルとして使いやすいよう、`App.vue` の薄い shell 化、feature-oriented な記事管理構成、記事操作 / タグ操作 / 詳細フォームの feature composable 分離、pure function 分離、API client / adapter 分離、i18n locale 分割、CSS ファイル分割を進めました
