...
- 無料枠公開・CI/CD構成は `docs/deployment/README.md` に整理済み。初期公開は Cloudflare Pages + Render Free Web Service + Supabase Free PostgreSQL を前提にし、`PORT` 対応、Actuator health の公開設定、managed DB 向け JDBC URL 前提、production profile の起動検証は実装済みで、deploy workflow だけ未着手
...
2. Cloudflare Pages / Render / Supabase の環境変数を実設定し、deploy workflow 化する
