...
- `.githooks/pre-push` や Playwright E2E smoke が `http://localhost:4173` / `http://localhost:18080` を使う場合、hook は E2E の前後で `docker compose -p articleshelf-e2e -f docker-compose.e2e.yml down -v --remove-orphans` を実行し、専用 stack を残さない。それでも port が埋まる場合は、別の dev server や手動起動 process を止める。`PLAYWRIGHT_USE_EXISTING_SERVER=1` は、意図して準備済み E2E server を再利用する場合だけ使う
