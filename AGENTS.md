...
- `.githooks/pre-push` や Playwright E2E smoke が `http://localhost:4173` / `http://localhost:18080` を使う場合は、既存 dev server や古い E2E stack を先に止める。`PLAYWRIGHT_USE_EXISTING_SERVER=1` は、意図して準備済み E2E server を再利用する場合だけ使う
